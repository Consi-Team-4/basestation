...
drop_enabled <- read.table("logs/Drop/2025-04-06T18-01-36-501567 drop enabled.csv", sep=",", col.names=col_names)
drop_enabled$seconds <- drop_enabled$micros / 1e6
enabled_start <- 306.8
drop_enabled <- subset(drop_enabled, seconds > enabled_start & seconds < enabled_start + 0.5)
...
drop_disabled <- read.table("logs/Drop/2025-04-06T18-02-32-985032 drop disabled.csv", sep=",", col.names=col_names)
drop_disabled$seconds <- drop_disabled$micros / 1e6
disabled_start <- 364.98
drop_disabled <- subset(drop_disabled, seconds > disabled_start & seconds < disabled_start + 0.5)
